...
public class GroupableTableHeaderUI extends BasicTableHeaderUI {
    ...
    private int spanReduction(ColumnGroup cg) {
        int ret = 0;

        for (int i = 0, size = cg.v.size(); i < size; i++) {
            if (cg.v.elementAt(i) instanceof TableColumn)
                ret++;
            else
                ret += spanReduction((ColumnGroup) cg.v.elementAt(i));
        }
        return ret;
    }
            ...
            if (cGroups != null) {
                ...
                while (cGroups.hasMoreElements()) {
                    ...
                    if (groupRect == null) {
                        ...
                        groupRect.width = d.width - spanReduction(cGroup);
                        groupRect.height = d.height;
                        h.put(cGroup, groupRect);
                    }
                    paintCell(g, groupRect, cGroup);
                    groupHeight += groupRect.height;
                    cellRect.height = size.height - groupHeight;//se ridica mai sus header-ul
...
//      return ;
    ...
    public Dimension getPreferredSize(JComponent c) {
        ...
        while (enumeration.hasMoreElements()) {
            TableColumn aColumn = (TableColumn) enumeration.nextElement();
            width = width + aColumn.getPreferredWidth() - 1;
        }
